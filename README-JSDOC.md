...
**This module is in alpha version. Incompatible changes may happen in future versions.**

Description
===========
This plugin determines which language you should serve to incoming request by looking several criteria of the request. Every aspect of the plugin can be configured with options. Sensible defaults are tried to be provided.
...
WHY
===
It is easy to determine language in hapi.js. So why is this plugin wirtten? We construct hapi applications from modules and most of them deal with i18n or i18n related stuff. We are tired of writing repetitive code for every application and some modules and decided to export this functionality as a hapi plugin.
...
1. Determines which languages are available in application. This happens one time during plugin registration.
2. Tries to find which language is prefered looking incoming request. This and other steps below happen in every request. Event for this step is configured by `options.onEvent` 
3. Matches requested language with available languages. If no match is found uses default locale or throws 404 according to options. 
...
Available languages are determined with methods in the following order. If one of the methods succeeds no other methods are tried. One or more steps may be cancelled via `options`. Available languages are searched one time during plugin registration.
...
###2. Requested Language(s)

One or more language may be preferred in requests. To determine most wanted language for every request following steps are taken in order given. One or more steps may be cancelled via `options`.
...
###3. Match Requested Language

Plugin tries to find first preferred language which is available in application:

1. If a match is found, language is determined.
...
3. If no 404 is thrown, default language is used as a result. Default language may set via `options.default`, otherwise first available language is used as default.
